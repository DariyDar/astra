...
must_haves:
  truths:
    ...
    - "Drizzle schema defines credentials and audit_trail tables"
  artifacts:
    - path: "docker-compose.yml"
      provides: "All service definitions with health checks"
      contains: "service_healthy"
    - path: "Dockerfile"
      provides: "Node.js app container build"
      contains: "node:22"
...
<task type="auto">
  <name>Task 1: Initialize TypeScript project with dependencies and Docker Compose stack</name>
  <files>
    package.json
    tsconfig.json
    docker-compose.yml
...
4. Create docker-compose.yml with ALL 5 services per user decision:
...
6. Create .dockerignore: node_modules, dist, .env, .git, .planning

7. Create .env.example with ALL required vars (documented, no real values):
...
8. Create .gitignore: node_modules, dist, .env, *.js.map

9. Create drizzle.config.ts pointing to src/db/schema.ts, PostgreSQL driver, DATABASE_URL from env.
...
IMPORTANT: No LiteLLM service (per user decision). Qdrant runs idle in Phase 1 (actual indexing in Phase 4) but must be healthy.
  ...
  <verify>
    ...
    <manual>docker compose config (validates compose file syntax)</manual>
  </verify>
  <done>TypeScript compiles clean, docker-compose.yml defines all 5 services with health checks, .env.example documents all required variables</done>
</task>

<task type="auto">
  <name>Task 2: Create database schema and config validation with bot/worker entry points</name>
  <files>
    src/config/env.ts
    src/db/schema.ts
    src/db/index.ts
...
NOTE: Bot and worker are minimal shells in this task. They prove the Docker services start and connect. Full functionality (logging, encryption, health checks) comes in Plan 02.
  ...
  <done>Config validation catches missing/invalid env vars with descriptive errors, DB schema defines credentials and audit_trail tables with proper indexes, bot responds to /start, worker starts and stays alive</done>
</task>
