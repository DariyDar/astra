...
- Do NOT make audit writes synchronous/blocking. Fire-and-forget.
  ...
  <verify>
    ...
    <automated>node -e "import('./dist/crypto/encrypt.js').then(m => { const k = Buffer.from('a'.repeat(64), 'hex'); const enc = m.encrypt('test', k); const dec = m.decrypt(enc, k); console.assert(dec === 'test', 'roundtrip failed'); console.log('encrypt/decrypt roundtrip OK'); })"</automated>
...
- Health alerts go to Telegram (the bot IS the monitoring channel per user decision).
  ...
  <verify>
    <automated>npx tsc --noEmit</automated>
    <automated>node -e "import('./dist/health/checker.js').then(m => { console.log('HealthChecker exports:', Object.keys(m)); console.assert(typeof m.HealthChecker === 'function', 'HealthChecker class missing'); console.log('health module loads OK'); })"</automated>
