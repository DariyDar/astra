...
2. **Create `src/integrations/monitors/calendar-reminders.ts`**: Export a `CalendarReminderMonitor` class.
   ...
   The class has:
   - `start()`: Starts a `node-cron` job running every 5 minutes (`*/5 * * * *`).
   - `stop()`: Stops the cron job.
   ...
   Import: `callClaude` from `../llm/client.js`, `NotificationDispatcher` from notifications, logger from logging, `node-cron`.
  </action>
  <verify>
...
- Calendar reminder checks every 5 minutes via Claude+MCP, dispatches 15-min reminders
