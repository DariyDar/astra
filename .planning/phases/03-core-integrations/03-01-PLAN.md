...
# NOTE: Google OAuth token storage deviation
# CONTEXT.md locked decision states "tokens stored encrypted in DB (same mechanism as Phase 1)".
# In practice, workspace-mcp manages its own OAuth flow and stores tokens in ~/.google-workspace-mcp/
# on the server (file-based, internal to the tool). This deviates from the original decision but is
# intentional: duplicating OAuth token management would replicate workspace-mcp internals for no gain.
# The user's key intent (secure, persistent token storage) is satisfied by workspace-mcp's mechanism.
# The DB-encrypted-token approach is deferred to a future phase if token portability becomes needed.
