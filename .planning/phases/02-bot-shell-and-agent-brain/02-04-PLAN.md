...
const adapter = new SlackAdapter({ botToken: 'xoxb-test', appToken: 'xapp-test', adminUserId: 'U000' });
if (adapter.channelType !== 'slack') throw new Error('channelType should be slack');
console.log('SlackAdapter construction OK');
"</automated>
    <manual>Verify SlackAdapter implements ChannelAdapter interface with start/stop/send/onMessage</manual>
  </verify>
  <done>Slack adapter created using Bolt Socket Mode with admin-only guard. Implements ChannelAdapter interface identical to Telegram adapter. Construction and type checks verified at runtime.</done>
</task>
...
Import SlackAdapter from `../channels/slack/adapter.js`. Import ChannelAdapter from `../channels/types.js`.
  ...
  <verify>
    ...
    <manual>Verify bot/index.ts conditionally creates SlackAdapter only when all 3 Slack env vars are present; .env.example documents Slack and Qdrant vars. INTEGRATION: Deploy without SLACK_* vars → bot starts in Telegram-only mode, logs confirm. If Slack app configured: deploy with SLACK_* vars → send DM in Slack → bot responds via Claude.</manual>
  </verify>
  <done>Slack adapter conditionally registered in bot entry point — present when configured, absent when not. Integration verified: bot starts in Telegram-only mode without Slack tokens; responds to Slack DMs when configured.</done>
</task>
...
- Deploy without SLACK_* env vars → bot starts, logs say "Telegram-only mode", Telegram still works
- Deploy with SLACK_* env vars → bot logs "Slack adapter configured", Slack Socket Mode connects
- Send DM to Astra in Slack → receive Claude-generated response (if Slack app created)
- Send DM from non-admin Slack user → no response (admin guard)
- Telegram continues to work alongside Slack (both adapters active)
</verification>
