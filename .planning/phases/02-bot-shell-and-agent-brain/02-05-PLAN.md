...
Import node-cron. Import NotificationPreferences from `../notifications/preferences.js`. Import DigestScheduler from `../notifications/digest.js`. Import NotificationDispatcher from `../notifications/dispatcher.js`.
  ...
  <verify>
    <automated>npx tsc --noEmit</automated>
    <manual>Verify /settings command is registered; digest cron job scheduled in bot; DigestScheduler exported with compileMorningDigest and deliverDigest; system prompt includes preference_update instructions; router scans for preference_update tags</manual>
  </verify>
  <done>Digest scheduler compiles and delivers morning digests. /settings command shows current notification preferences. Natural language preference updates work: user says "set task deadlines to urgent on Slack" and Claude outputs preference_update tag, router parses and executes the update, user sees confirmation. Digest cron job scheduled in bot process at 8 AM daily. Notification system fully wired into bot lifecycle with graceful shutdown.</done>
</task>
