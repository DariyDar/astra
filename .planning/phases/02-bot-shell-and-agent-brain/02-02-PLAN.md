...
const mem = new ShortTermMemory(redis);
const testMsg = { id: 'test-1', channelType: 'telegram', channelId: 'test-ch', userId: 'u1', role: 'user', text: 'integration test', timestamp: new Date() };
await mem.store('test-ch', testMsg);
const result = await mem.getRecent('test-ch', 1);
if (result.length < 1 || result[0].text !== 'integration test') throw new Error('ShortTermMemory store/retrieve failed');
await mem.clear('test-ch');
await redis.quit();
console.log('ShortTermMemory integration OK');
"</automated>
    <manual>Verify ShortTermMemory and MediumTermMemory classes export correctly with all methods. Verify MediumTermMemory stores and retrieves from PostgreSQL.</manual>
  </verify>
  <done>Short-term memory stores messages in Redis with 24h TTL and retrieves recent messages — verified with live Redis. Medium-term memory persists messages to PostgreSQL with date range and keyword queries. Both compile and pass integration checks.</done>
</task>
...
IMPORTANT: The embedding call in `search()` is synchronous with the search. For `store()`, the caller (context builder) will handle async embedding to avoid blocking the response path.
  ...
  <verify>
    <automated>npx tsc --noEmit && node --import tsx -e "
import { initEmbedder, embed, getEmbeddingDimension } from './src/memory/embedder.js';
await initEmbedder();
const vec = await embed('тестовое сообщение');
if (vec.length !== getEmbeddingDimension()) throw new Error('Embedding dimension mismatch: ' + vec.length);
if (vec.length !== 384) throw new Error('Expected 384 dimensions, got ' + vec.length);
console.log('Embedder integration OK: ' + vec.length + ' dimensions');
"</automated>
    <manual>Verify LongTermMemory.ensureCollection() creates Qdrant collection without error. Verify store + search round-trip returns the stored message.</manual>
  </verify>
  <done>Embedding pipeline initializes a local multilingual ONNX model (384 dimensions, no API key) — verified with live embedding. Long-term memory stores vectors in Qdrant and retrieves semantically similar messages. All integration checks pass.</done>
</task>
...
- ShortTermMemory: store → getRecent round-trip through live Redis
- MediumTermMemory: store → getRecent round-trip through live PostgreSQL
- Embedder: initEmbedder() downloads model, embed('text') returns 384-dim vector
- LongTermMemory: ensureCollection() creates Qdrant collection, store → search returns stored message
</verification>
