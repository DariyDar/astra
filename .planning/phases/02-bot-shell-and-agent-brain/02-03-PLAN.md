...
import { buildSystemPrompt } from './src/brain/system-prompt.js';
const promptRu = buildSystemPrompt('ru');
const promptEn = buildSystemPrompt('en');
if (!promptRu.includes('ru') && !promptRu.includes('Russian') && !promptRu.includes('русск')) throw new Error('System prompt does not reference Russian language');
if (!promptEn.includes('en') && !promptEn.includes('English')) throw new Error('System prompt does not reference English language');
console.log('System prompt OK: ru=' + promptRu.length + ' chars, en=' + promptEn.length + ' chars');
"</automated>
    <manual>Verify TelegramAdapter implements ChannelAdapter; buildContext assembles from all 3 memory tiers; buildSystemPrompt returns language-specific prompt</manual>
  </verify>
  <done>Telegram adapter wraps grammY Bot into ChannelAdapter interface with admin guard. Context builder assembles three-tier memory into structured text with graceful degradation. System prompt defines Astra's persona with language awareness — verified at runtime.</done>
</task>
...
Use existing imports: `callClaude` from `../llm/client.js`, `createRequestLogger` from `../logging/correlation.js`, `writeAuditEntry` from `../logging/audit.js`, `logger` from `../logging/logger.js`. Use `{ Redis }` named import from `ioredis`.
  ...
  <verify>
    ...
    <manual>Verify MessageRouter is exported with process/start/stop methods; bot/index.ts creates router with Telegram adapter and memory tiers. CRITICAL INTEGRATION: Deploy to server, send a Telegram message — bot must respond with a Claude-generated contextual reply. Verify /start and /health still work.</manual>
  </verify>
  <done>Message router processes incoming messages through language detection, three-tier context assembly, and Claude conversation. Bot entry point refactored to use adapter pattern with memory initialization. Existing /start and /health commands preserved. Integration verified: bot responds to Telegram messages via Claude.</done>
</task>
...
Integration checks (live infrastructure — deploy to server):
- Send "Привет" in Telegram → bot responds in Russian
- Send "Hello" in Telegram → bot responds in English
- Send two messages in a row → second response references the first (context works)
- Send /start → welcome message (preserved from Phase 1)
- Send /health → health status (preserved from Phase 1)
- Check logs: messages being stored in Redis, PostgreSQL, and Qdrant
- Bot startup: embedder initializes, Qdrant collection created, no errors in logs
</verification>
