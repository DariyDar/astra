...
type ClockifyReportType = 'summary' | 'who_tracked' | 'who_missing'
...
async function executeClockifyReport(req: ClockifyReportRequest): Promise<ClockifyReportResult> {
  const startTime = Date.now()
  const apiKey = process.env.CLOCKIFY_API_KEY
  const wsId = process.env.CLOCKIFY_WORKSPACE_ID
  if (!apiKey || !wsId) throw new Error('CLOCKIFY_API_KEY or CLOCKIFY_WORKSPACE_ID not set')
...
async function main(): Promise<void> {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [briefingTool, searchEverywhereTool, clockifyReportTool],
  }))
