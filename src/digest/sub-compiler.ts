...
[ИМЯ ПРОЕКТА]
• факт 1 (автор, ссылка если есть)
• факт 2
(пустая строка между проектами)

ПРАВИЛА:
• Только факты: решения, блокеры, статусы, числа, имена, версии. Никаких оценок.
• Стендапы — извлекай РЕЗУЛЬТАТЫ (что сделано, блокеры), не "состоялся стендап".
• CS/Support — проблема пользователей, кол-во, критичность.
• Production-updates — дедлайны, статусы майлстоунов.
• 2-4 буллета на проект максимум. Если нет данных — не включай проект.
• Имена: короткие русские (Дарий, Настя, Сергей).
• Ссылки: включай Slack URL в скобках после факта.
