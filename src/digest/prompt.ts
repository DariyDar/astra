...
• Стендапы, дейли — если в данных есть стендап/агенда стендапа, извлеки РЕЗУЛЬТАТЫ: что обсуждали, какие решения, какие блокеры, какие статусы. Не пиши "состоялся стендап" — пиши что на нём решили/обсудили.
• QA-отчёты (из email) — обязательно связывай с проектом: кол-во багов, приоритеты, критичные проблемы. Кратко: "QA: 3 P1, 8 P2, ключевой — краш на iOS 17" <a href="url">(отчёт)</a>
• CS/Customer Support сообщения (Slack) — если есть обратная связь от CS, кратко: что за проблема, сколько пользователей, критичность.
• Production updates (каналы #ac-production-updates, #production-updates) — дедлайны, статусы майлстоунов, кто что сдал/не сдал.
...
• Slack — суммировать КЛЮЧЕВЫЕ обсуждения: решения, блокеры, запросы, статусы. НЕ пересказывать каждое сообщение. Данные из ВСЕХ каналов (проектные, стендапы, production-updates) важны.
...
• Ссылки: КАЖДЫЙ буллет должен иметь кликабельную ссылку на источник (если URL доступен). Формат: <a href="url">описание</a>. Ссылка должна быть компактной и спрятанной за описательный текст.
  Примеры: <a href="url">тред</a>, <a href="url">письмо</a>, <a href="url">задача</a>, <a href="url">встреча</a>, <a href="url">отчёт</a>
• НЕ используй markdown-таблицы (| --- |). Они НЕЧИТАЕМЫ в Telegram. Всегда буллеты.
...
• Финальный формат каждого буллета: "суть события (<a href=\"url\">источник</a>)" — компактно, со ссылкой.
